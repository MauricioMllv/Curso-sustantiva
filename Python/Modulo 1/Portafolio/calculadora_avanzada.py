...
for boton in botones:
    if boton == 'C':
        ...
        tk.Button(ventana, text=boton, width=4, height=2, command=calcular_potencia).grid(row=row_val, column=col_val)
    elif boton == '√':
        tk.Button(ventana, text=boton, width=4, height=2, command=calcular_raiz_cuadrada).grid(row=row_val, column=col_val)
    else:
